...
public class JobTrackingIT
{
    ...
    /**
     * This test demonstrates the task table deletion abandoned issue covered by SCMOD-
     * @throws Exception 
     */
    @Test(enabled = true)
    public void testDatabaseDeadlock() throws Exception
    {
        System.out.println(
            "--- Starting 'testDatabaseDeadlock'");
        final JobsApi jobsApi = createJobsApi();
        ...
        final JobTrackingWorkerReporter jobTrackingWorkerReporter = new JobTrackingWorkerReporter();
        System.out.println("---'testUpdateInReverse' Sending status update messages for job : " + jobId);
        jobTrackingWorkerReporter.reportJobTaskProgress(jobId + ".51*", 0);
        ...
        for (int index = 0; index < 5; index++) {
            final Thread t = new Thread(() -> {
                try {
                    while (true) {
                        jobTrackingWorkerReporter.reportJobTaskComplete(jobId + "." + r.nextInt(High + 1));
                    }
                } catch (java.util.NoSuchElementException e) {
                } catch (final Exception ex) {
                    System.out.println("--twoSubTaskReportProgressTest Exception : " + ex);
        ...
        for (int index = 0; index < 10; index++) {
            final Thread t = new Thread(() -> {
                try {
                    int numberOfUpdatesProcessed = 0;
                    while (true) {
                        jobTrackingWorkerReporter.reportJobTaskProgress(jobId + "." + r.nextInt(High + 1), 0);
                    }
                } catch (java.util.NoSuchElementException e) {
                } catch (final Exception ex) {
                    System.out.println("--twoSubTaskReportProgressTest Exception : " + ex);
